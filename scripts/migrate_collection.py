...
COLLECTION_JSON_URL = (
    "https://uk1s3.embassy.ebi.ac.uk/public-datasets/24agents.aicell.io/collection.json"
)
...
async def fetch_collection_json():
    async with httpx.AsyncClient(headers={"Connection": "close"}) as client:
        ...
        ), f"Failed to fetch collection.json from {COLLECTION_JSON_URL}"
...
async def fetch_collection_config():
    ...
    async with httpx.AsyncClient(headers={"Connection": "close"}) as client:
        ...
        assert (
            response.status_code == 200
        ), f"Failed to fetch collection config from {COLLECTION_CONFIG_URL}"
        return json.loads(response.text)
...
async def download_manifest(rdf_source):
    async with httpx.AsyncClient(headers={"Connection": "close"}) as client:
        ...
        assert (
            response.status_code == 200
        ), f"Failed to fetch manifest from {rdf_source}"
        return yaml.safe_load(
            response.text.replace("!<tag:yaml.org,2002:js/undefined>", "")
        )
    ...
    while retries < max_retries:
        try:
            async with httpx.AsyncClient(
                timeout=DEFAULT_TIMEOUT, headers={"Connection": "close"}
            ) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"Rate limit hit for {url}, retrying after {retry_delay} seconds..."
                    )
...
async def upload_file(
    ...
    if not file_path.startswith("http"):
        ...
        try:
            await artifact_manager.get_file(artifact_id, file_url)
            # remove the file with https name
            await artifact_manager.remove_file(artifact_id, file_url)
            logger.info(f"File with url file name has been removed {file_url}")
        except Exception:
            pass

    try:
        file_url = await artifact_manager.get_file(artifact_id, file_path)
    ...
    while retries < max_retries:
                ...
                async with client.stream("GET", file_url) as response:
                    if response.status_code == 200:
                        ...
                        if "Content-Length" in response.headers:
                            headers["Content-Length"] = response.headers[
                                "Content-Length"
                            ]
                        upload_response = await client.put(
                            put_url, data=response.aiter_bytes(), headers=headers
                        )
                        if upload_response.status_code == 200:
                            ...
                            logger.warning(
                                f"Rate limit hit for {file_url}, retrying after {retry_delay} seconds..."
                            )
                            await asyncio.sleep(retry_delay)
                        else:
                            logger.warning(
                                f"Failed to upload {artifact_id}: {file_path}, status code: {upload_response.status_code}, {upload_response.text}"
                            )
                            return
                    else:
                        logger.exception(
        ...
        if retries < max_retries:
            await asyncio.sleep(retry_delay * retries)  # Exponential backoff
    logger.error(
        f"Failed to upload {artifact_id}: {file_path} after {max_retries} retries."
    )
                ...
                if isinstance(file["architecture"], str):
                    ...
                    if file["architecture"]["source"]:
                        await upload_file(
                            ...
                            artifact_id,
                            base_url,
                            file["architecture"]["source"].split(":")[0],
                        )
            await upload_file(
                artifact_manager,
                artifact_id,
...
async def verify_and_reupload_files(
    artifact_manager, artifact_id, base_url, full_manifest, rdf_source
):
    ...
    if missing_files:
        logger.info(
            f"Re-uploading {len(missing_files)} missing files for {artifact_id}"
        )
        ...
        for file_path in missing_files:
            try:
                download_weight = (
                    1
                    if any(
                        file_path in weights_file.get("source", "")
                        for weights_file in weights.values()
                        if weights_file
                    )
                    else 0
                )
...
async def migrate_collection(
    ...
    if reset_stats:
        ...
        if confirmation != "reset-stats":
            ...
            logger.warning(
                "STATS RESET CONFIRMED BY USER - proceeding with destructive operation"
            )
    ...
    ), "SANDBOX_ZENODO_ACCESS_TOKEN is not set"
    ...
    try:
        collection = await artifact_manager.read("24agents.aicell.io")
        ...
        if update_reviewers:
            ...
            logger.info(
                "Collection exists and update_reviewers=False, skipping permission updates"
            )

    except Exception as e:
        logger.info(f"Collection doesn't exist, creating new one: {e}")
        collection = await artifact_manager.create(
            alias="24agents.aicell.io",
            type="collection",
            manifest={
                k: collection_json[k]
                for k in collection_json
    ...
    semaphore = asyncio.Semaphore(
        CONCURENT_TASKS
    )  # Limit to CONCURENT_TASKS concurrent tasks
        ...
        async with semaphore:
            ...
            try:
                ...
                if reset_stats:
                    ...
                    logger.info(
                        f"STATS_BACKUP - {dataset_id}: view_count={view_count}, download_count={download_count}"
                    )
                ...
                if verify_files:
                    ...
                    try:
                        ...
                        logger.info(
                            f"Verification complete for {dataset_id}: {verification_result['total_files']} total files, {verification_result['missing_files']} missing files re-uploaded"
                        )
                        if verification_result["issues"]:
                            logger.warning(
                                f"Issues found for {dataset_id}: {verification_result['issues']}"
                            )
                    except Exception as e:
                ...
                if skip_migrated and not verify_files:
                    logger.info(
                        f"{full_manifest['type']} {dataset_id} already migrated."
                    )
                    return
                elif skip_migrated and verify_files:
                    logger.info(
                        f"{full_manifest['type']} {dataset_id} already migrated, files verified."
                    )
                    return
            ...
            artifact = await artifact_manager.create(
                type=full_manifest.get("type"),
                alias=dataset_id,
                parent_id="bioimage-io/24agents.aicell.io",
                manifest=full_manifest,
                stage=True,
                overwrite=True,
            )
            ...
            await upload_file(
                artifact_manager, artifact.id, base_url, item["rdf_source"]
            )
            try:
                await upload_files(
            ...
            try:
                ...
                logger.info(
                    f"{artifact.type} {dataset_id} migrated, file count: {artifact.file_count}"
                )
            except Exception as e:
                logger.error(
                    f"Failed to migrate, failed to commit {artifact.type} {dataset_id}: {e}"
                )
    ...
    tasks = [
        migrate_dataset(
            item,
            skip_migrated=skip_migrated,
            edit_existing=edit_existing,
            reset_stats=reset_stats,
            verify_files=verify_files,
        )
        for item in collection_json["collection"]
    ]
