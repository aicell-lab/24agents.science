...
export const useKernelManager = ({ clearRunningState, onKernelReady, autoStart = false }: UseKernelManagerProps) => {
  ...
  const processBuffer = useCallback((
    ...
    // If the buffer ends with a newline, we have complete lines.
    // If not, the last part is incomplete and should remain in the buffer.
    // However, if we want "one log per print", print("foo") sends "foo\n".
    // So we should get ["foo", ""].
    ...
    lines.forEach(line => {
      // Skip empty lines if desired, but print("") is valid.
      // The user said "empty lines become their own messages".
      // If we have "foo\n\nbar", we get ["foo", "", "bar"].
      // The middle "" is an empty line.
      // If we want to avoid "empty message boxes", we can filter out empty strings.
      // But maybe the user just meant "don't break 'foo' into 'f', 'oo'".
      // Let's emit everything that is a complete line.
      
      // Actually, if the user dislikes empty lines, let's filter them out ONLY if they are truly empty.
      // But print() produces a newline.
      // If we emit "foo", the UI shows "foo".
      // If we emit "", the UI shows an empty box.
      // Let's filter out empty lines for now to be safe, or maybe just whitespace-only lines?
      // No, print("") should probably show something or nothing.
      // Let's filter out completely empty lines to reduce noise.
      if (line.length > 0) {
        const output = {
          type,
          content: line,
  ...
  const loadWebPythonKernel = useCallback(async () => {
    if (kernelManagerRef.current) {
      console.log('[Web Python Kernel] Using cached kernel manager');
    ...
    try {
      ...
      console.log('[Web Python Kernel] Creating KernelManager with worker URL:', workerUrl);
      ...
      console.log('[Web Python Kernel] KernelManager created successfully');
    ...
    return async (code: string, callbacks?: ExecuteCodeCallbacks, _timeout?: number) => {
      ...
      try {
        ...
        if (stdoutBufferRef.current) {
          // If there is remaining text without a newline, emit it
  ...
  const startKernel = useCallback(async () => {
    ...
    if (isInitializingRef.current) {
      console.log('[Web Python Kernel] Already initializing, skipping duplicate call');
      return;
    }

    // If already ready, don't reinitialize
    if (currentKernelIdRef.current && kernelManagerRef.current?.manager) {
      console.log('[Web Python Kernel] Kernel already initialized, skipping');
      return;
    }

    // Mark as initializing
    console.log('[Web Python Kernel] Starting kernel initialization...');
    ...
    try {
      ...
      console.log('[Web Python Kernel] Status set to starting, loading kernel module...');

      // Load the kernel module
      const { manager, KernelMode, KernelLanguage, KernelEvents } = await loadWebPythonKernel();
      console.log('[Web Python Kernel] Kernel module loaded, creating kernel...');
      ...
      console.log('[Web Python Kernel] Kernel created with ID:', kernelId);
      ...
      manager.onKernelEvent(kernelId, KernelEvents.KERNEL_BUSY, () => {
        console.log('[Web Python Kernel] Kernel is BUSY');
        setKernelStatus('busy');
      });

      manager.onKernelEvent(kernelId, KernelEvents.KERNEL_IDLE, () => {
        console.log('[Web Python Kernel] Kernel is IDLE');
        setKernelStatus('idle');
      });
      ...
      setKernelStatus('idle');
      setIsReady(true);
      console.log('[Web Python Kernel] Kernel ready! ID:', kernelId);
      ...
      isInitializingRef.current = false;
      console.log('[Web Python Kernel] Initialization complete');
    } catch (error) {
      clearTimeout(initTimeout);
  ...
  const mountFolder = useCallback(async (dirHandle: any) => {
    ...
    for (let i = 0; i < maxRetries; i++) {
      ...
      if (manager && kernelId) {
        ...
        if (kernel && kernel.kernel && kernel.kernel.mountFS) {
          try {
