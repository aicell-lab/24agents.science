...
const ArtifactDetails = () => {
  ...
  const handleDownload = () => {
    ...
    if (artifactId) {
      let downloadUrl = `https://hypha.aicell.io/24agents-science/artifacts/${artifactId}/create-zip-file`;
...
<Button
        ...
        >
          ...
          {isFullscreen && selectedResource?.manifest?.type === 'model' && showModelRunner && (
            ...
            >
              <img 
                src="/static/img/24agents-science-icon-small.svg" 
                alt="24Agents.Science"
                style={{
                  height: '40px',
                  width: '40px',
                }}
          ...
          {selectedResource?.manifest?.type === 'model' && showModelRunner && (
            <Box sx={{ mt: isFullscreen ? 0 : { xs: 1, sm: 2, md: 3 } }}>
              <ModelRunner
                ...
                className="w-full"
                modelUrl={`https://hypha.aicell.io/24agents-science/artifacts/${selectedResource.id.split("/").pop()}/create-zip-file${version && version !== 'latest' ? `?version=${version}` : ''}`}
              />
            </Box>
          )}
